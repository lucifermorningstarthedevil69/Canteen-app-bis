...
import { useCollection, useFirestore, useUser, addDocumentNonBlocking, updateDocumentNonBlocking, deleteDocumentNonBlocking, useMemoFirebase } from "@/firebase";
...
function MenuForm({
  ...
  onSave: (item: Omit<MenuItem, "id" | "image" | "imageHint" | "canteenInchargeId"> & { id?: string }) => void;
  onClose: () => void;
}) {
  ...
  const handleSubmit = () => {
    onSave({ id: menuItem?.id, name, description, price });
  };
...
export default function MenuPage() {
  ...
  const handleSave = (item: Omit<MenuItem, "id" | "image" | "imageHint"| "canteenInchargeId"> & { id?: string }) => {
    if (!user) {
    ...
    if (item.id) {
      ...
      updateDocumentNonBlocking(menuItemDoc, item);
      toast({ title: "Item Updated", description: `${item.name} has been updated.` });
    } else {
      // Add
      const newItem: Omit<MenuItem, "id"> = {
        ...item,
        image: "https://picsum.photos/seed/new/400/300", // Default image for new items
        ...
        <CardContent>
            <Table>
              ...
              <TableBody>
                {isLoading && (
                    <TableRow>
                        <TableCell colSpan={5} className="text-center">Loading menu...</TableCell>
                    </TableRow>
                )}
